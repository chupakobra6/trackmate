...
| STEP-005 | `кандидат` | REQ-013 | После пользовательского ревью финализировать prod-миграцию, выполнить DB dry-run на доступной PostgreSQL и получить approval. | Проверить backup/restore/migrate/rollback шаги и что нет destructive SQL для текущих данных. | dry-run локальной миграции + approved command sequence |
| STEP-006 | `отложено` | REQ-013 | После approval выкатить обновление на production. | Подтвердить backup, миграции, запуск сервисов, smoke-check новых topic bindings. | prod command outputs, post-deploy checks |
