...
@pytest.mark.asyncio
async def test_acknowledge_alert_by_other_user_does_not_change_state(session, monkeypatch) -> None:
    workspace_repo = WorkspaceRepository(session)
    today_repo = TodayRepository(session)
    workspace = await workspace_repo.get_or_create_workspace(-1004004004005, "Group", "UTC")
    participant = await workspace_repo.register_participant(workspace.id, 601, "igor", "Igor")
    task = await today_repo.create_daily_task(
        workspace_id=workspace.id,
        participant_id=participant.id,
        owner_user_id=participant.user_id,
        task_date=workspace.created_at.date(),
        text="Task",
        today_card_message_id=21,
    )
    alert = await today_repo.get_or_create_alert(task.id, AlertKind.DAY_CLOSED_PENDING_REPORT)
    alert.telegram_message_id = 95
    await session.commit()
    ...
    refreshed_alert = await session.get(type(alert), alert.id)
    assert refreshed_alert is not None
    assert refreshed_alert.acknowledged_at is None
    assert refreshed_alert.telegram_message_id == 95
