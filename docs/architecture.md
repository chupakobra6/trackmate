...
Ошибки отдельной отправки Telegram (400/403) сохраняются в `worker_delivery_retries` по виду операции и ID сущности. Следующие независимые карточки и этапы Tick продолжаются. Повторы ждут 1, 2, 4, 8, 16, 32 и 64 минуты; после восьмой неудачи запись приостанавливается для разбора. Успех подтверждается существующей предметной записью, затем технический повтор удаляется. Захват alert/progress/auto-close сразу исключает отложенные и приостановленные записи, поэтому цикл не захватывает один отказ снова без ожидания.

Механизм покрывает карточки, напоминания и автоматическое закрытие рутин, обновление лидерборда, начальные и повторные вопросы по целям, итог периода, alerts и progress. Продуктовые сроки проверяются до технического повтора: истёкший вопрос закрывается даже без первой доставки, отложенное напоминание сохраняет прежнюю карточку. Очистка через `messagecleanup` по-прежнему работает best effort; сбои БД, неизвестные ошибки, отмена и общая недоступность Telegram остаются явными ошибками прохода.

Запрос, заведомо не записанный в соединение, допускает обычный ограниченный повтор. Если `sendMessage` мог дойти, а подтверждение оборвалось, автоматический повтор приостанавливается: сначала нужно сопоставить чат и сохранённое состояние. Даже при отмене worker пытается сохранить такой исход отдельным коротким контекстом. Доставка и PostgreSQL не образуют общей транзакции: аварийное завершение процесса или недоступность БД между отправкой и сохранением оставляют окно неопределённости. Существующие leases и компенсационное удаление сохранены; это не обещание exactly-once. Разбор и возобновление — в [операциях](operations.md#повторы-фоновой-доставки).
