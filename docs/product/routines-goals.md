...
19. Clean up ordinary unfinished input after 24 hours silently, deleting the stored bot prompt and known process messages. Two-week goal reviews use the specialized lifecycle below.
...
27. Keep each unanswered two-week goal review available for 72 hours: replace the first prompt with exactly one identical retry after 24 hours, then remove the retry and persist the review as skipped at 72 hours. Do not overlap the final period review with an open two-week review.
