...
Для ревью-прогонов, где нужно оставить визуальные примеры в Telegram, cleanup не
...
- `13-goals-weekly-final.jsonl.tmpl`: seasonal goals setup, weekly review и
  final review в `Цели`.
- `14-goal-nudge.jsonl.tmpl`: редкая deterministic-вставка про сезонные цели при
...
Time-based сценарии `12` и `13` удобнее запускать частями: сначала пользовательский
setup, затем `control/clock` + `control/tick`, затем оставшиеся шаги сценария.
Иначе runner может начать ждать карточку, которая создается только worker tick.
Для goal nudge в `14` перед запуском нужны уже сохраненные активные цели и дата,
seed которой проходит deterministic nudge gate; в live-прогоне использовался
