...
- до 20:00 создается задача дня, а после 20:00 при отсутствии записи — итог дня;
...
- напоминание подтверждается кнопкой `👀 Понял` только его адресатом; чужой
  callback и cross-workspace payload проверяются PostgreSQL integration tests;
...
- неотвеченный двухнедельный вопрос повторяется один раз через 24 часа и
  окончательно исчезает как пропущенный через 72 часа;
...
- `15-day-summary.jsonl.tmpl`: вечерний итог дня со статусом `🔸 Средне`, публикацией
  и синхронизацией правки в `Прогресс`.
- `16-routine-auto-close.jsonl.tmpl`: точечный auto-close рутины; итоговая карточка
  остается в теме, alert приходит reply с адресатом/source-link и закрывается через `👀 Понял`.
- `17-goals-review-retry-skip.jsonl.tmpl`: точечный lifecycle неотвеченного
  двухнедельного вопроса: один retry через 24 часа и cleanup/skip через 72 часа.
...
Для `15` сначала очисти тестовое пространство, затем выставь время не раньше 20:00
в часовом поясе группы. Для `Europe/Moscow` подойдёт `2026-07-12T17:00:00Z`; после
сценария верни часы системному времени.
Для focused-сценария `16` во время ожидания карточки выставь 08:00 следующего
локального дня и вызови `/control/tick`; после появления карточки выставь 00:00
еще через день и снова вызови `/control/tick`. Полный Telegram E2E для этой
проверки не нужен.
