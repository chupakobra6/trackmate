...
- неотвеченный двухнедельный вопрос повторяется один раз через 24 часа и
  окончательно закрывается как пропущенный через 71 час; ошибка удаления не
  блокирует финал сезона;
- финал сезона принимает несколько сообщений и становится постоянной карточкой
  только после кнопки `✅ Завершить итог`;
- редкая вставка про цели на сезон появляется в `Сегодня` только при активных целях.

Технические повторы отправки проверяются в `internal/worker/retry_integration_test.go` с локальной БД: ошибочный элемент не задерживает независимый, отложенный повтор переживает перезапуск, успешная отправка не повторяется, бюджет и общая ошибка сохраняются. Ошибки Telegram намеренно подставляются локально; live-сценарий подтверждает обычный вид карточек и конечное состояние, но не имитирует сбой рабочего Telegram.
...
- `13-goals-weekly-final.jsonl.tmpl`: настройка целей на сезон, вопросы по целям и
  многосообщениевый итог периода с явным завершением в `Цели`; final prompt
  отвечает на исходные цели, а постоянная карточка явно называет участника.
...
- `17-goals-review-retry-skip.jsonl.tmpl`: точечный lifecycle неотвеченного
  двухнедельного вопроса: один retry через 24 часа и cleanup/skip через 71 час.
- `18-late-task-report.jsonl.tmpl`: auto-fail редактирует исходную задачу без
  кнопки, а `Результат` в alert принимает один поздний отчёт без смены failed-статуса
  и без второго Progress event.
...
проверки не нужен.
Для focused-сценария `18` после создания задачи выставь 12:01 следующего
локального дня и вызови `/control/tick`. Сценарий проверяет failed-карточку,
